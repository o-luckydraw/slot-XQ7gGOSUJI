...
const normalizePrize = (raw: any, index: number): Prize => {
  ...
  const remaining = Number.isFinite(rawRemaining)
    ...
    : quantity;
  const consumed = Math.max(0, Number(raw?.consumed) || 0);
...
const defaultPrizes: Prize[] = [
  { id: makePrizeId(), rank: 1, name: '테스트 상품 1', quantity: 10, remaining: 10, consumed: 0, requiresShipping: false },
  { id: makePrizeId(), rank: 2, name: '테스트 상품 2', quantity: 100, remaining: 100, consumed: 0, requiresShipping: false },
  { id: makePrizeId(), rank: 3, name: '테스트 상품 3', quantity: 100, remaining: 100, consumed: 0, requiresShipping: false },
  { id: makePrizeId(), rank: 4, name: '테스트 상품 4', quantity: 100, remaining: 100, consumed: 0, requiresShipping: false },
  { id: makePrizeId(), rank: 5, name: '테스트 상품 5', quantity: 100, remaining: 100, consumed: 0, requiresShipping: false },
];
...
export const useDrawStore = create<DrawStore>((set, get) => ({
  ...
  updatePrize: (index, updated) => {
    ...
    const current = newPrizes[index];

    let finalUpdated = { ...updated };

    // 전체수량 변경 시 남은수량 = 전체수량으로 직접 세팅 (소진수량은 별도 관리)
    if ('quantity' in updated && updated.quantity !== undefined) {
      const newQuantity = Math.max(0, Number(updated.quantity) || 0);
      finalUpdated.remaining = newQuantity;
    }
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        const mergedItems: Prize[] = prizes.map((localPrize, index) => {
          ...
          if (!remotePrize) {
            // 신규 상품: consumed=0, remaining=quantity
            return {
              ...localPrize,
              rank: index + 1,
          ...
          // 기존 상품: consumed은 Firebase 값을 보존 (추첨이 동시에 일어날 수 있음)
          const safeLocalRemaining = Math.max(0, Math.min(safeLocalQuantity, Number(localPrize.remaining) || 0));
          const remoteConsumed = Math.max(0, Number(remotePrize.consumed) || 0);
          ...
          return {
            ...
            rank: index + 1,
            quantity: safeLocalQuantity,
            remaining: safeLocalRemaining,
            consumed: remoteConsumed,
            requiresShipping: !!localPrize.requiresShipping,
          };
        });
    ...
    const txResult = await runTransaction(db, async (transaction) => {
      ...
      for (let i = 0; i < drawCount; i += 1) {
        ...
        if (!isTestMode) {
          ...
          target.consumed = (target.consumed || 0) + 1;
        }
      }
